import pygame
from random import uniform #random decimal in range
...
class Sprite(pygame.sprite.Sprite):
    def __init__(self,pos,surf,groups,z=WORLD_LAYERS['main']):
        super().__init__(groups) #super is used when I don't want to change the parent class attributes
        self.image=surf
        self.rect=self.image.get_frect(topleft=pos)
        self.z=z #determines which sprite is in front
        self.y_sorted=self.rect.centery
        self.hitbox=self.rect.copy()
class GrassSprite(Sprite):
    def __init__(self,pos,surf,groups,biome,monsters,level):
        self.biome=biome #sand has grass on background
        super().__init__(pos,surf,groups,WORLD_LAYERS['main' if biome!='sand' else 'background'])
        self.y_sorted-=40
        self.biome=biome
        self.monsters=monsters.split(',')
        self.level=level #monsters and their levels
...
class AnimatedSprite(Sprite):
    ...
    def animate(self,dt):
        self.frame_index+=Animation_speed*dt #changes frames, dt to maintain frame rate
        self.image=self.frames[int(self.frame_index%4)]
    def update(self,dt):
        self.animate(dt) #runs each frame
class BorderSprite(Sprite):
    def __init__(self,pos,surf,groups):
        super().__init__(pos,surf,groups)
        self.hitbox=self.rect.copy() #to handle collisions seperately
class TransitionSprite(Sprite):
    def __init__(self,pos,size,target,groups):
        surf=pygame.Surface(size)
        super().__init__(pos,surf,groups)
        self.target=target #target map

class CollidableSprite(Sprite):
    def __init__(self,pos,surf,groups):
        super().__init__(pos,surf,groups)
        self.hitbox=self.rect.inflate(0,-self.rect.height*0.4) #to provide some leeway
class MonsterSprite(pygame.sprite.Sprite):
    def __init__(self,pos,frames,groups,monster,index,pos_index,entity,apply_attack,create_monster):
        ...
        self.animation_speed=Animation_speed+uniform(-1,1)  #floating point equivalent of randint
        ...
        #sprite setup
        super().__init__(groups) #to add the sprites to certain groups
    ...
    def animate(self,dt):
        ...
        if self.state=='attack' and self.frame_index>=len(self.frames['attack']):
            ...
            self.state='idle' #back to idle after attack
        ...
        if self.highlight:
            white_surf=pygame.mask.from_surface(self.image).to_surface() #silhouette
...
class MonsterNameSprite(pygame.sprite.Sprite):
    def __init__(self,pos,monster_sprite,groups,font):
        ...
        text_surf=font.render(monster_sprite.monster.name,False,COLOURS['black'])
        padding=10
    ...
    def update(self,_):
        if not self.monster_sprite.groups():   #kill only removes sprite from groups
            self.kill()
...
class MonsterLevelSprite(pygame.sprite.Sprite):
    ...
    def update(self,_):
        self.image.fill(COLOURS['white'])
        text_surf=self.font.render(f'Lvl: {self.monster_sprite.monster.level}',False,COLOURS['black'])
        text_rect=text_surf.get_frect(center=(self.rect.width/2,self.rect.height/2))
        self.image.blit(text_surf,text_rect)
        bar(self.image,self.xp_rect,self.monster_sprite.monster.xp,self.monster_sprite.monster.level_up,COLOURS['black'],COLOURS['white'],0)
...
class MonsterStatsSprite(pygame.sprite.Sprite):
        ...
        def update(self,_):
            ...
            for index,(value,max_value) in enumerate(self.monster_sprite.monster.get_info()):
                colour=(COLOURS['red'],COLOURS['blue'],COLOURS['gray'])[index]
                if index<2:   #energy,health
                    text_surf=self.font.render(f'{int(value)}/{int(max_value)}',False,COLOURS['black'])
                    ...
                    self.image.blit(text_surf,text_rect)
                    bar(self.image,bar_rect,value,max_value,colour,COLOURS['black'],2)
                else:
                    init_rect=pygame.FRect((0,self.rect.height-2),(self.rect.width,2))
                    bar(self.image,init_rect,value,max_value,colour,COLOURS['white'],0)
...
class AttackSprite(AnimatedSprite):
    ...
    def animate(self,dt):
        ...
        if self.frame_index<len(self.frames):
            self.image=self.frames[int(self.frame_index)]  #we only want one animation
        else:
            self.kill()    #destroying sprite
    def update(self,dt):
        self.animate(dt)
