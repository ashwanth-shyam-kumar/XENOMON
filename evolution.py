...
class Evolution:
    def __init__(self,frames,start_monster,end_monster,font,end_evolution,star_frames):
        ...
        self.timers={
            ...
            'end':Timer(2000,func=end_evolution)
        }
        #star animation
        ...
        #text
        self.start_text_surf=font.render(f'WHAT! {start_monster} is evolving!',False,COLOURS['black'])
        self.end_text_surf=font.render(f'Your {start_monster} evolved into {end_monster}!',False,COLOURS['black'])

    def display_stars(self,dt):
        self.frame_index+=20*dt
        if self.frame_index<len(self.star_frames):
            frame=self.star_frames[int(self.frame_index)]
            rect=frame.get_frect(center=(WIDTH/2,HEIGHT/2))
    ...
    def update(self,dt):
        ...
        if not self.timers['start'].active:
            ...
            if self.tint_amount<255:
                rect=self.start_monster_surf.get_frect(center=(WIDTH/2,HEIGHT/2))
                ...
                pygame.draw.rect(self.display_surface,COLOURS['white'],text_rect.inflate(20,20),0,5)
                self.display_surface.blit(self.start_text_surf,text_rect)
            else:
                rect=self.end_monster_surf.get_frect(center=(WIDTH/2,HEIGHT/2))
